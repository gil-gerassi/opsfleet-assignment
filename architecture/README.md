...
| 6 | Staging | Pre-prod - same architecture as Prod, held close to its exact config (not just scaled down), so it's a valid pre-release test environment. |
| 7 | Production | Customer-facing workloads + production database only. |

3 platform/governance accounts (1–3) + 1 shared-services account (4) + 3 environment accounts (5–7) = **7 accounts**. No other accounts are recommended at launch.
...
- **Isolation:** hard account boundaries per environment; Production restricted to human access, all changes via CI/CD.
- **Billing:** consolidated invoice; AWS Budgets + Cost Anomaly Detection per account, so a run-away Dev cost is visible immediately, not buried in the total - FinOps best practice.
